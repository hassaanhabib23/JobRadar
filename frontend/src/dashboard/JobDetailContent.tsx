/**
 * One job, in full — the score breakdown, timeline, reminder and notes.
 *
 * Extracted from `routes/JobDetail.tsx` so there is exactly one implementation
 * of "what a job's full detail looks like": the standalone `/app/jobs/:id`
 * page renders it inside `AppShell`/`Column`, and the dashboard's inline detail
 * panel renders it directly in the third column, with no page navigation.
 *
...
import { MAXIMUMS, SEGMENTS, ScoreBar, TIER_TONE } from './ScoreBar'
...
export function JobDetailContent({
  ...
  jobId: number
  /** The standalone `/app/jobs/:id` page needs a way back; the inline detail
      panel doesn't — the list is still right there in the middle column. */
  backLink?: boolean
}) {
